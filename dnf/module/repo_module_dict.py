...
class RepoModuleDict(OrderedDict):
    ...
    def get_includes(self, name, stream):
        ...
        for module_version in version_dependencies:
            ...
            if conf.enabled._get() or \
                    (conf.enabled_defaults._get() and default_stream == module_version.stream):
            ...
            for repo_module_stream in repo_module.values():
                if (repo_module.conf.enabled._get() and
                    repo_module.conf.stream._get() == repo_module_stream.stream) or \
                        (repo_module.conf.enabled_defaults._get() and
    ...
    def reset_to_default(self, module_version, save_immediately=False):
    ...
    def get_info(self, module_spec):
        ...
        default_stream = module_version.repo_module.defaults.peek_default_stream()
        default_str = " [d]" if module_version.stream == default_stream and \
                      conf.enabled_defaults._get() else ""
        enabled_str = ""
